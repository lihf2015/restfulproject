...
<meta http-equiv="expires" content="0">
	...
	<script type="text/javascript">
       ...
       function ddelete(obj) {
		    alert();
			var tds = $(obj).parent().parent().find('td');
		    $.ajax("<@s.url'/user/deleteUser?userId=" + tds.eq(0).text()+ "'/>");
			window.location.reload();
		}
								...
								$("#userTable > tbody").html(""); //每次加载的时候先设为空
									$.each(data.event,function(i, value) {
										$("#userTable > tbody").append("<tr><td>"
												...
												+ "<a href=\"javascript:void(0)\" onclick=\"ddelete(this);\" class=\"btn btn-danger btn-xs active\" role=\"button\">Delete</a>"
												+ "</td>"
												+ "</tr>");
									});
